...
> **当前状态：尚未在真实环境演练（UNVERIFIED）。**
>
> 本轮 review 核验时 8 项清单全部为未勾选，且**没有任何演练记录**。
> 本节此前只列出「完成 Postgres 备份与恢复演练」一项，属于无证据的
> 流程要求——按本项目「事实 / 规则 / 叙事三层不可混淆」的原则，
> 这里如实记录为**未验证**，而不是默认视为已完成。
>
> 该状态意味着：**当前没有任何证据表明备份可恢复**。
> 上线前必须在真实（或与生产同构的）环境执行下列步骤并回填结果。

演练步骤（可执行，逐条记录真实输出）：

| # | 步骤 | 命令 / 操作 | 预期 | 实际结果 | 日期 | 执行人 |
|---|---|---|---|---|---|---|
| 1 | 生成备份 | `./scripts/backup-postgres.example.ps1`（或 `pg_dump -Fc`） | 产出非空 dump 文件 | 未执行 | — | — |
| 2 | 校验 dump 可读 | `pg_restore --list <dump>` | 列出表结构，无报错 | 未执行 | — | — |
| 3 | 建隔离库 | `createdb restore_drill` | 库创建成功 | 未执行 | — | — |
| 4 | 恢复 | `pg_restore -d restore_drill <dump>` | 退出码 0 | 未执行 | — | — |
| 5 | 行数比对 | 对 `charts` / `people` / `ziwei_charts` / `liuyao_charts` / `users` / `share_snapshots` 比对源库与恢复库行数 | 逐表一致 | 未执行 | — | — |
| 6 | 应用连通 | 以恢复库为 `DATABASE_URL` 启动 standalone，访问 `/api/health/ready` | 返回 ready | 未执行 | — | — |
| 7 | 关键读路径 | 登录后读取一份历史命盘与一份分享链接 | 数据可读、内容正确 | 未执行 | — | — |
| 8 | 计时 | 记录步骤 4 的耗时 | 满足 RTO 目标 | 未执行 | — | — |
| 9 | 清理 | `dropdb restore_drill` | 演练库删除 | 未执行 | — | — |

RPO / RTO 目标（需产品与运维确认后填入，当前**未定义**）：

- RPO（可接受数据丢失窗口）：待填写
- RTO（可接受恢复时长）：待填写
- 备份频率与保留期：待填写

**验收标准**：9 行全部填入真实结果与日期，且步骤 5 逐表行数一致，
方可勾选 §11.3 的「完成 Postgres 备份与恢复演练」。
