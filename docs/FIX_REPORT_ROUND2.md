...
## 14. B 档收口（第三轮追加，详见 `docs/FIX_REPORT_ROUND3.md`）

第二轮留下的 B 档 16 项在第三轮全部有终态（10 项落地、6 项取证后判定不做，其中 B3/B8 虽判「不改」但各自带上门禁与差异测试；另有 2 个本轮新发现的故障已修），
其中**两条对本报告自身结论的更正**必须记录：

| 位置 | 本报告原结论 | 第三轮实测 |
|---|---|---|
| §11.5 | 新增的 Tailwind 守护测试「已实测有效」 | 结论只对一半：该测试**自己**在 JSDoc 里写了危险类名字面量并对本文件自我豁免，于是守护自身成了新的污染源，`npx playwright test` 再次整站样式构建失败（本轮实测 24 例全灭）；已在 `35aabe5` 修复（去字面量 + 取消自我豁免）。从提交顺序看（`ec950cb` 修 CSS → `1b3962c` 加守护），§11.4 记的「0/24 → 11 passed」是在**加入该守护之前**测的，所以没有暴露这个问题 |
| §11.6 | 剩余 e2e 失败为「`e2e.invalid` 占位符 + 移动端并行超时，与代码改动无关」 | 不完整：CI 用 `node .next/standalone/server.js`（NODE_ENV=production），`instrumentation` 的生产校验器会因占位 `AUTH_SECRET`（15 字符）与未设 `SHARE_STORE_DRIVER` **直接拒绝启动**，`/api/health` 返回 500 → 24 例在 CI 下从未有机会运行。已在 `108ed12` 修，并以 curl 实测启动恢复 200 |

B 档逐项结果（细节、量化对比与未验证声明见 `FIX_REPORT_ROUND3.md`）：
B1 / B2 / B4 / B5 / B6 / B7 / B12 / B14 / B15 / B16 已落地并带测试；
B3（CSP）经生产 standalone 实测确认**不可在不放弃 PPR/静态化的前提下消除**，改为把结论固化成双向门禁；
B8 补三术 LLM 行为差异测试后**判定不合并**；B9 / B10 / B11 / B13 以实测取证判定不做
（其中 B11 的「113 vs 113」前提已失效，实测 19 vs 94）。
